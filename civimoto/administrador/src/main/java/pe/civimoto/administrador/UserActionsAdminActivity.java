...
public class UserActionsAdminActivity extends DocumentAdminActivity {
    ...
    private void showUsers(){
        ...
        LinearLayout intro=card();intro.addView(tx("Usuarios registrados",22,Color.WHITE,true));intro.addView(tx("Cada cuenta tiene acciones separadas: Aprobar, Suspender, Eliminar y Notificar.",13,MUTED,false));body().addView(intro);
        LinearLayout list=card();list.addView(tx("Cargando usuarios…",14,MUTED,false));body().addView(list);loadUsersActions(list);
    }
        ...
        backend().rpc("cm_admin_user_directory",new JSONObject(),new Backend.Callback(){
            public void ok(Object x){
                ...
                for(int i=0;i<a.length();i++){
                    ...
                    LinearLayout c=card();c.addView(tx(name,16,Color.WHITE,true));c.addView(tx(u.optString("email","—")+" · "+u.optString("phone","Sin teléfono"),12,MUTED,false));c.addView(tx("Estado: "+status+" · Calificación: "+u.optString("rating","—"),12,GOLD2,true));
