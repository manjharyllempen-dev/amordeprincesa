...
/** Panel CiviMoto de choferes con foto de perfil, documentos, aprobación y eliminación. */
public class DocumentAdminActivity extends InteractiveFlowActivity {
    ...
    @Override void screenDrivers(){
        ...
        LinearLayout intro=card();intro.addView(tx("Choferes registrados",22,Color.WHITE,true));intro.addView(tx("Revisa foto de perfil, identidad, DNI, tarjeta de propiedad y mototaxi antes de aprobar.",13,MUTED,false));body().addView(intro);
    ...
    private void loadDriverDirectory(LinearLayout box){
        ...
        backend().rpc("cm_admin_driver_directory",new JSONObject(),new Backend.Callback(){public void ok(Object x){
    ...
    private void addDriverCard(LinearLayout box,JSONObject d){
        ...
        LinearLayout c=card();
        ImageView face=profileImage();c.addView(face,new LinearLayout.LayoutParams(dp(112),dp(112)));loadProfilePhoto(d.optString("face_photo_path"),face);
        c.addView(tx(name+" · "+d.optString("plate","Sin placa"),17,Color.WHITE,true));c.addView(tx("Estado: "+status+" · "+(verified?"✓ documentos verificados":"documentos pendientes"),12,verified?GREEN:ORANGE,true));
        ...
        Button delete=btn("Eliminar chofer",false);delete.setTextColor(RED);c.addView(delete);delete.setOnClickListener(v->confirmDelete(id,name,box));box.addView(c);
    }
    ...
    private void showDocumentMenu(JSONObject d,String name){String[] labels={"Foto DNI","Tarjeta de propiedad","Foto del mototaxi","Foto de rostro / perfil"};String[] paths={d.optString("dni_photo_path"),d.optString("property_card_photo_path"),d.optString("vehicle_photo_path"),d.optString("face_photo_path")};new AlertDialog.Builder(this).setTitle("Documentos de "+name).setItems(labels,(dialog,which)->openImage(labels[which],paths[which])).setNegativeButton("Cerrar",null).show();}
    private void openImage(String title,String path){if(path==null||path.trim().isEmpty()){toast("El conductor todavía no cargó este documento.");return;}toast("Abriendo documento…");new AdminStorage(backend()).download(path,new Backend.Callback(){public void ok(Object x){byte[] data=(byte[])x;Bitmap bm=BitmapFactory.decodeByteArray(data,0,data.length);if(bm==null){toast("No se pudo leer la imagen.");return;}ScrollView sc=new ScrollView(DocumentAdminActivity.this);LinearLayout root=new LinearLayout(DocumentAdminActivity.this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(dp(10),dp(10),dp(10),dp(10));ImageView im=new ImageView(DocumentAdminActivity.this);im.setAdjustViewBounds(true);im.setScaleType(ImageView.ScaleType.FIT_CENTER);im.setImageBitmap(bm);root.addView(im,new LinearLayout.LayoutParams(-1,-2));sc.addView(root);new AlertDialog.Builder(DocumentAdminActivity.this).setTitle(title).setView(sc).setPositiveButton("Cerrar",null).show();}public void error(String m){toast(m);}});}
