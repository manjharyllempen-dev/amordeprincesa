...
/** Registro de pasajero con número de DNI, foto de DNI y foto de rostro/perfil. */
public class PassengerDocumentActivity extends LiveRatingActivity {
    ...
    @Override void screenRegister(){
        ...
        addPhotoControl(c,"face","📷 Tomar foto de mi cara / perfil");

        Button create=btn("Crear cuenta y guardar identificación",true);
        Button back=btn("Ya tengo cuenta",false);
        c.addView(create);c.addView(back);body().addView(c);

        create.setOnClickListener(v->{
            String n=name.getText().toString().trim(),e=email.getText().toString().trim(),p=phone.getText().toString().trim(),d=dni.getText().toString().trim();
            if(n.isEmpty()||e.isEmpty()||p.isEmpty()||d.isEmpty()||pass.length()<8){toast("Completa nombre, correo, teléfono, DNI y contraseña.");return;}
            if(!photos.containsKey("dni")||!photos.containsKey("face")){toast("Toma la foto del DNI y la foto de tu cara/perfil.");return;}
            create.setEnabled(false);
            backend().register(n,e,pass.getText().toString(),"usuario",new Backend.Callback(){
                public void ok(Object x){backend().login(e,pass.getText().toString(),"usuario",new Backend.Callback(){
                    public void ok(Object y){uploadDocuments(p,d,create);}
    ...
    private void addPhotoControl(LinearLayout c,String kind,String title){
        Button b=btn(title,false);
        TextView status=tx("Pendiente",12,android.graphics.Color.rgb(255,175,45),true);
        status.setPadding(dp(4),0,0,dp(5));
        c.addView(b);c.addView(status);photoStatus.put(kind,status);
        b.setOnClickListener(v->takePhoto(kind));
    }
    ...
    private void takePhoto(String kind){
        try{
            ...
            values.put(MediaStore.Images.Media.DISPLAY_NAME,"civimoto_passenger_"+kind+"_"+System.currentTimeMillis()+".jpg");
    ...
    @Override protected void onActivityResult(int requestCode,int resultCode,Intent data){
        ...
        try{
            ...
            if((bytes==null||bytes.length==0)&&data!=null&&data.getExtras()!=null&&data.getExtras().get("data") instanceof Bitmap){
                Bitmap b=(Bitmap)data.getExtras().get("data");ByteArrayOutputStream os=new ByteArrayOutputStream();b.compress(Bitmap.CompressFormat.JPEG,88,os);bytes=os.toByteArray();
            }
            if(bytes==null||bytes.length==0)throw new Exception("sin imagen");
            photos.put(pendingKind,bytes);
            TextView s=photoStatus.get(pendingKind);if(s!=null){s.setText("✓ Foto capturada");s.setTextColor(android.graphics.Color.rgb(35,190,105));}
    ...
    private byte[] readAndCompress(InputStream in)throws Exception{
        ...
        int w=b.getWidth(),h=b.getHeight(),max=Math.max(w,h);
        if(max>1800){float f=1800f/max;b=Bitmap.createScaledBitmap(b,Math.max(1,(int)(w*f)),Math.max(1,(int)(h*f)),true);}
        ByteArrayOutputStream os=new ByteArrayOutputStream();b.compress(Bitmap.CompressFormat.JPEG,88,os);return os.toByteArray();
    }

    private void uploadDocuments(String phone,String dni,Button create){
        PassengerStorage st=new PassengerStorage(backend());
        String uid=backend().getUserId();String base="passengers/"+uid+"/";
        st.uploadJpeg(base+"dni.jpg",photos.get("dni"),new Backend.Callback(){public void ok(Object a){
            st.uploadJpeg(base+"face.jpg",photos.get("face"),new Backend.Callback(){public void ok(Object b){
                saveDocumentPaths(phone,dni,base,create);
            }public void error(String m){failUpload(create,m);}});
        }public void error(String m){failUpload(create,m);}});
    }

    private void saveDocumentPaths(String phone,String dni,String base,Button create){
        try{
            JSONObject p=new JSONObject()
                    .put("p_document_number",dni)
                    .put("p_dni_photo_path",base+"dni.jpg")
                    .put("p_face_photo_path",base+"face.jpg")
                    .put("p_phone",phone);
            backend().rpc("cm_passenger_save_documents",p,new Backend.Callback(){
                public void ok(Object x){toast("Cuenta creada correctamente.");screenHome();}
                public void error(String m){failUpload(create,m);}
            });
