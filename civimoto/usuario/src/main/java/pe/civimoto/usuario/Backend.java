...
public class Backend {
    ...
    // Se reemplaza automáticamente por el nuevo proyecto Supabase exclusivo CiviMoto.
    ...
    public Backend(Context c){
        prefs=c.getSharedPreferences("civimoto_pasajero_session_v1",Context.MODE_PRIVATE);
    ...
    public void register(String fullName,String email,String password,String requestedRole,Callback cb){io.execute(()->{try{
        ensureConfigSync();JSONObject body=new JSONObject().put("full_name",fullName.trim()).put("email",email.trim().toLowerCase()).put("password",password).put("role",requestedRole);
    ...
    private void ensureConfigSync()throws Exception{
        ...
        HttpURLConnection c=open("GET",CONFIG_URL,false);int code=c.getResponseCode();String txt=read(c,code>=200&&code<300);
        if(code<200||code>=300)throw new Exception(extractMessage(txt));
        JSONObject o=new JSONObject(txt);supabaseUrl=o.getString("supabase_url");publishableKey=o.getString("publishable_key");
    ...
    private void ensureFreshTokenSync()throws Exception{
        ...
        storeSession(asObject(requestSync("POST",supabaseUrl+"/auth/v1/token?grant_type=refresh_token",body,false)));
    }
    private JSONObject getProfileSync()throws Exception{
        ...
        JSONArray a=raw instanceof JSONArray?(JSONArray)raw:new JSONArray();if(a.length()==0)throw new Exception("No se encontró el perfil CiviMoto.");return a.getJSONObject(0);
    }
    private void storeSession(JSONObject s)throws Exception{
        accessToken=s.getString("access_token");refreshToken=s.getString("refresh_token");userId=s.getJSONObject("user").getString("id");
        expiresAtMs=System.currentTimeMillis()+s.optLong("expires_in",3600L)*1000L;
        prefs.edit().putString("access",accessToken).putString("refresh",refreshToken).putString("uid",userId).putLong("expires",expiresAtMs).apply();
    }
    private Object requestSync(String method,String absoluteUrl,JSONObject body,boolean auth)throws Exception{
        HttpURLConnection c=open(method,absoluteUrl,auth);if(body!=null&&!"GET".equals(method)){byte[] bytes=body.toString().getBytes(StandardCharsets.UTF_8);c.setDoOutput(true);c.setFixedLengthStreamingMode(bytes.length);try(OutputStream os=c.getOutputStream()){os.write(bytes);}}
        int code=c.getResponseCode();String txt=read(c,code>=200&&code<300);
        if(code<200||code>=300){if(code==401&&auth&&refreshToken!=null){expiresAtMs=1L;ensureFreshTokenSync();return requestSync(method,absoluteUrl,body,true);}throw new Exception(extractMessage(txt));}
        if(txt==null||txt.trim().isEmpty())return new JSONObject().put("ok",true);String s=txt.trim();if(s.startsWith("["))return new JSONArray(s);if(s.startsWith("{"))return new JSONObject(s);return s;
    }
    private HttpURLConnection open(String method,String absoluteUrl,boolean auth)throws Exception{
        HttpURLConnection c=(HttpURLConnection)new URL(absoluteUrl).openConnection();c.setConnectTimeout(15000);c.setReadTimeout(20000);c.setRequestMethod(method);c.setRequestProperty("Content-Type","application/json");c.setRequestProperty("Accept","application/json");
        if(publishableKey!=null)c.setRequestProperty("apikey",publishableKey);if(auth&&accessToken!=null)c.setRequestProperty("Authorization","Bearer "+accessToken);if(!"GET".equals(method))c.setRequestProperty("Prefer","return=representation");return c;
    }
    ...
    private String friendly(Exception e){String m=e.getMessage();if(m==null||m.trim().isEmpty())return"No se pudo conectar con el backend exclusivo de CiviMoto.";if(m.contains("Invalid login credentials"))return"Correo o contraseña incorrectos.";if(m.contains("Email not confirmed"))return"Confirma tu correo antes de ingresar.";if(m.contains("civimoto-backend-not-created"))return"Backend CiviMoto independiente pendiente de activación.";return m;}
