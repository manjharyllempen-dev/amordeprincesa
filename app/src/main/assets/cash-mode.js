...
choosePayment = async function(method){
  ...
  try{
    ...
    let ref=null;
    if(selectedMethod==='manual_transfer'){
      ref=prompt('Referencia o comprobante de transferencia (opcional):','')||null;
    }
    await api('/rest/v1/rpc/set_own_order_payment_method',{
      method:'POST',
      body:JSON.stringify({p_order_id:id,p_method:selectedMethod,p_reference:ref})
    });
    go(11);
  }catch(e){
...
async function renderCashPayment(){
  ...
  const bankInfo=transferReady?`<div class="card"><b>Transferencia manual</b>${s.bank_alias?`<p><b>Alias:</b> ${esc(s.bank_alias)}</p>`:''}${s.bank_cbu?`<p><b>CBU:</b> ${esc(s.bank_cbu)}</p>`:''}${s.bank_holder?`<p><b>Titular:</b> ${esc(s.bank_holder)}</p>`:''}${s.bank_name?`<p><b>Banco:</b> ${esc(s.bank_name)}</p>`:''}<p class="small">Realizá la transferencia y después confirmá el pedido. El administrador verificará el pago manualmente.</p></div><button class="btn soft" onclick="choosePayment('manual_transfer')">🏦 PAGAR POR TRANSFERENCIA</button>`:'';
...
async function renderCashSettings(){
  ...
  <div class="msg ok">Mercado Pago está deshabilitado. La app no usa credenciales ni checkout de Mercado Pago.</div>
...
setTimeout(()=>{renderCashPayment();renderCashSettings();removeMercadoPagoText();},50);
