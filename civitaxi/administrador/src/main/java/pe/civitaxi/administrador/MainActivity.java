...
public class MainActivity extends Activity {
    int purple=Color.rgb(82,38,168), dark=Color.rgb(48,16,111), yellow=Color.rgb(255,196,0); TextView status;
    ...
    void showSplash(){LinearLayout s=new LinearLayout(this);s.setOrientation(LinearLayout.VERTICAL);s.setGravity(Gravity.CENTER);s.setPadding(dp(30),dp(30),dp(30),dp(30));s.setBackground(bg(dark,0));s.addView(logo(260,210));TextView brand=t("CiviTaxi",42,Color.WHITE,true);brand.setGravity(Gravity.CENTER);s.addView(brand);LinearLayout mr=new LinearLayout(this);mr.setGravity(Gravity.CENTER);mr.addView(t("Mototaxi",22,Color.WHITE,true));mr.addView(logo(58,44));s.addView(mr);TextView sub=t("Administrador · Control total",15,Color.WHITE,false);sub.setGravity(Gravity.CENTER);s.addView(sub);setContentView(s);new Handler(Looper.getMainLooper()).postDelayed(this::build,1400);}
    void addHeader(LinearLayout r){LinearLayout h=new LinearLayout(this);h.setGravity(Gravity.CENTER_VERTICAL);h.addView(logo(74,58));LinearLayout text=new LinearLayout(this);text.setOrientation(LinearLayout.VERTICAL);text.addView(t("CiviTaxi",29,Color.WHITE,true));LinearLayout sub=new LinearLayout(this);sub.setGravity(Gravity.CENTER_VERTICAL);sub.addView(t("Administrador · Mototaxi",14,Color.WHITE,false));sub.addView(logo(42,32));text.addView(sub);h.addView(text);r.addView(h);}
