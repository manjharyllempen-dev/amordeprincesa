...
public class Backend {
    ...
    public Backend(Context c){prefs=c.getSharedPreferences("civimoto_session",Context.MODE_PRIVATE);supabaseUrl=prefs.getString("url",null);publishableKey=prefs.getString("key",null);accessToken=prefs.getString("access",null);refreshToken=prefs.getString("refresh",null);userId=prefs.getString("uid",null);role=prefs.getString("role",null);expiresAtMs=prefs.getLong("expires",0L);}
    ...
    private JSONObject getProfileSync()throws Exception{Object raw=requestSync("GET",supabaseUrl+"/rest/v1/profiles?id=eq."+userId+"&select=id,role,full_name,phone,email,account_status,rating",null,true);JSONArray a=raw instanceof JSONArray?(JSONArray)raw:new JSONArray();if(a.length()==0)throw new Exception("No se encontró el perfil.");return a.getJSONObject(0);}
    ...
    private String extractMessage(String txt){try{JSONObject o=new JSONObject(txt);String m=o.optString("message","");if(m.isEmpty())m=o.optString("msg","");if(m.isEmpty())m=o.optString("error_description","");if(m.isEmpty())m=o.optString("error","");if(!m.isEmpty())return m;}catch(Exception ignored){}return(txt==null||txt.trim().isEmpty())?"Error de conexión.":txt;}
    private String friendly(Exception e){String m=e.getMessage();if(m==null||m.trim().isEmpty())return"No se pudo conectar con Civimoto.";if(m.contains("Invalid login credentials"))return"Correo o contraseña incorrectos.";if(m.contains("Email not confirmed"))return"Confirma tu correo antes de ingresar.";return m;}
    private void clearSession(){accessToken=null;refreshToken=null;userId=null;role=null;expiresAtMs=0L;prefs.edit().remove("access").remove("refresh").remove("uid").remove("role").remove("expires").apply();}
